...
test_input_container_no_seccontext_allowed {
    input := { "review": input_review_no_seccontext}
    results := violation with input as input
    count(results) == 1
}
...
input_review_no_seccontext = {
    "object": {
        "metadata": {
            "name": "nginx"
        },
        "spec": {
            "containers": input_containers_one_no_seccontext
      }
    }
}
...
input_containers_one_no_seccontext = [
{
    "name": "nginx",
    "image": "nginx"
}]
